...
1. 初始化一个物理内存管理器PMM，该管理器分配释放物理内存。对应的策略有很多种。
2. 创建物理页表。
3. 对物理内存分配进行检查。
4. 创建二级页表。
5. 创立段机制的的映射和取消。
6. 初始化GDT表，写入段描述符。
7. 并将GDT起始地址写入GDTR寄存器。
8. 将PDT起始地址写入CR3，设置CR0_PG标志位，设置分页机制使能
...
错误信息是`unexpected trap in kernel`，这是因为在IDT指定的位置找不到相应的中断处理程序，所以11号没有被处理，最后报错。
